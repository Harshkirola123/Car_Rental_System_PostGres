...
import { Renter } from "../../renters/renter.entity"; // Assuming you have a User model
...
export const refreshToken = async (
  ...
  try {
    ...
    if (decoded.role == "USER") {
      user = await userRepository.findOne({ where: { _id: decoded._id } });
    } else {
      user = await adminRepository.findOne({ where: { _id: decoded._id } });
    }
    // console.log(user);
    if (!user) {
